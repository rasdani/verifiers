"""
SWE-RL Utilities for patch handling and diff operations.

Adapted from https://github.com/SWE-bench/SWE-bench/blob/main/swebench/harness/utils.py
and https://github.com/R2E-Gym/R2E-Gym/tree/main/src/r2egym/commit_models
"""
...
def strip_comment_lines(patch: str) -> str:
    """Remove comment lines from patch."""
...
def extract_minimal_patch(model_patch):
    """
    Extract minimal patch by:
    * Removing trailing non +/- lines and trailing whitespace per line per hunk
    * Recalculating hunk start/end position and diff delta
    * Returns new patch
    """
...
class LineGroup(BaseModel):
    ...
    def lr_lines(self) -> List[Line]:
        return [line for line in self.all_lines if line.type in [LineType.DELETED, LineType.CONTEXT]]
...
class UniHunk(BaseModel):
    ...
    def edited_entities(self) -> set[Entity]:
        return self.modified_entities.union(self.added_entities).union(self.deleted_entities)
    ...
    def num_function_entities(self) -> int:
        return sum(entity.type == EntityType.FUNCTION for entity in self.edited_entities)
    ...
    def edit_transcends_single_location(self) -> bool:
        return (self.num_function_entities + self.num_class_entities > 1) or (self.num_method_entities > 1)
...
class FileDiffHeader(BaseModel):
    ...
    def is_test_file(self) -> bool:
        return self.path.endswith("_test.py") or self.path.startswith("test_") or "tests" in self.path.split("/")
...
class FileDiff(BaseModel):
    ...
    def _parse_hunk_header(self, header_line: str) -> UnitHunkDescriptor:
        """Parse hunk header line to extract ranges."""
        ...
        return UnitHunkDescriptor(old_range=old_range, new_range=new_range, section=section)

    def _parse_range(self, range_str: str) -> Range:
        """Parse range string like '1,4' or '1' to Range object."""
        if "," in range_str:
